...
def create_inverted_index(user_weekday = 1, user_puid = 41, user_doid = 24, user_hour = 0, user_minutes = 21, filename = 'yellow_tripdata_2018-01_sample.csv'):
    try :
        ...
        convert_to_duration_udf = udf(lambda pickup_date, dropoff_date: get_duration(pickup_date, dropoff_date), IntegerType())
        ...
        inverted_index = fields_df.select(
            convert_to_weekday_udf("pickup_datetime").alias("weekday"),
            convert_to_hour_udf("pickup_datetime").alias("hour"),
            "pickup_id",
            "dropoff_id",
            avg(convert_to_duration_udf("pickup_datime", "dropoff_datime")).alias("average_duration"),
            avg("amount").alias("average_amount")
        ).groupBy("weekday", "hour", "pickup_id", "dropoff_id")
